...
type CordinatesWithNoCountry struct {
	Name string  `json:"name"`
	Lat  float64 `json:"lat"`
	Lon  float64 `json:"lon"`
}

func GetCordsCityOnly(requestedLocation string) (string, float64, float64, string) {
...
func GetCordsCityAndCountry(requestedLocation string) (string, float64, float64) {
	...
	var cords []CordinatesWithNoCountry
	err = json.NewDecoder(response.Body).Decode(&cords)
	if err != nil {
		panic(err)
	}
